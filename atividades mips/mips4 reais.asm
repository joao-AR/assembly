...
.text
	main: 
		...
		lwc1 $f4,zero # vai receber a soma de i + j de todos elementos da matriz
		...
		addi $s2, $zero, 8 #s2 = tamanho dos elementos em bytes = 8 bytes = sizeof(float)
...
add_diagonal_secundaria:
	...
	calcular_endereco_elemento_matriz: #calcula o endereço do elemento mat[i][j]
		...
		sll $v0, $v0, 4    # [(i * n col) + j] * 4(calcula o deslocamento em bytes para se chegar no elemento mat[i][j])
